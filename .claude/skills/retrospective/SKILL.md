---
name: retrospective
description: End-of-session save discipline for India AI Brain users. Runs 2-pass leak-check (Rule 6: no internal-shorthand, no internal-paths, no personal-build references). Saves session learning so the firm gets smarter with every use.
allowed-tools: Bash, Read, Glob
---

# /retrospective — End-of-Session Save Discipline (India)

Saves what the India AI Brain learned this session. Runs at session end. The firm accumulates knowledge across sessions without leaking personal context.

## 2-Pass Leak-Check (Rule 6 — execute BEFORE any save)

### Pass 1 — Internal-shorthand codes + internal paths
```bash
# Check for personal entity-aliasing entity codes
...
# Check for user-home dot-directory references
...
```bash
# Check for external architecture-name references
...
# Check for publisher personal-vs-business markers (NOT the publisher credit line in README — that's allowed)
...
- What legal domains were touched (criminal, civil, constitutional, corporate, DPDP, BCI compliance)
- What courts or jurisdictions were referenced
- Any errors or blockers encountered

### Step 2: Run 2-pass leak-check (above)

### Step 3: If passes leak-check, write session summary
Save to `.ailawfirm-india/sessions/` with timestamp. Format:
```markdown
...
## Output Format
```markdown
## /retrospective — India AI Brain · Session Save
