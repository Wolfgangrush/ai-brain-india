...
The advocate's filing reality: Indian courts accept Word, not markdown. The firm reads markdown. Both audiences exist. So every drafted document produces two files, atomically, in the matter's drafts directory.
...
## Lite-mode exception (low-RAM Macs · opt-in)

Before pairing, check whether the firm is in lite mode. Lite mode is enabled when EITHER:

- The file `~/.ailawfirm-india/lite.flag` exists, OR
- The environment variable `AILAWFIRM_LITE` is set to `1`

```bash
[ -f ~/.ailawfirm-india/lite.flag ] || [ "$AILAWFIRM_LITE" = "1" ] && echo "LITE" || echo "NORMAL"
```

If lite mode is ON:

- Write the `.md` file only — DO NOT run pandoc per draft.
- Tell the advocate:
  > ✍️ Draft saved as markdown (lite mode ON).
  > When ready to file/email, batch-generate .docx:
  >   `ailawfirm-india finalize <matter-folder>`

If lite mode is OFF (default), proceed with normal md+docx pairing below.

Lite mode exists because per-draft pandoc generation is the largest CPU consumer per drafting turn on fanless 8GB Macs (M1/M2 Air). Batched generation at end of session produces bit-identical .docx (pandoc is deterministic) with one CPU burst instead of per-keystroke heat.
...
- **Registry reality:** Indian court e-filing portals + physical filing windows accept .docx / .pdf, not .md. A markdown-only draft cannot be filed.
